...
import { todayISO, type AppState, type LessonRecord, type Profile } from '@catechise/shared';
...
export function ProfileProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<ProfileContextValue>(() => {
    ...
    return {
      ...
      addProfile: (p) => persist((s) => ({ profiles: [...s.profiles, p], activeProfileId: p.id })),
        ...
        persist((s) => ({
          ...
          profiles: s.profiles.map((p) =>
            p.id === s.activeProfileId
              ? { ...p, completed: { ...p.completed, [lessonId]: { date: todayISO(), ...record } } }
              : p,
          ),
        })),
